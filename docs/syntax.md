...
JsonLang 程序是一个 JSON 对象，包含以下顶级字段：
...
JsonLang 支持通过 `comment` 语句添加注释，有两种形式：
...
JsonLang提供一系列内置的数组操作语句，无需依赖外部模块即可处理数组：
...
JsonLang 完全支持递归函数调用，可以实现复杂的算法，如阶乘、斐波那契数列等。
