...
这个文档详细描述了JLang编程语言的语法。JLang是一种基于JSON的编程语言，它允许您使用熟悉的JSON结构编写程序，同时提供变量、循环、条件、函数等编程功能。
...
JLang 支持通过 `comment` 语句添加注释，有两种形式：
...
JLang提供一系列内置的数组操作语句，无需依赖外部模块即可处理数组：
...
JLang 完全支持递归函数调用，可以实现复杂的算法，如阶乘、斐波那契数列等。
