...
```bash
# 容错模式运行
jlang --ignore-non-critical-errors examples/my_program.jl
```
...
```bash
# 只检查错误，不执行
jlang --check examples/my_program.jl
```
...
```bash
# 调试模式运行
jlang --debug examples/my_program.jl
```

这些新增的命令行选项使得JLang在开发、测试和生产环境中都能更灵活地运行。
